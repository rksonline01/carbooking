...
const addNotificationValidationRules = (req, res) => {
	...
	/*	body("image")
			.custom((value, { req }) => {
				...
				return req.__('admin.email_notification.please_select_image', { value, location, path });
			}),*/
		body('pages_descriptions.' + DEFAULT_LANGUAGE_CODE + '.subject').optional().trim().notEmpty()
			.withMessage((value, { req, location, path }) => {
				return req.__('admin.email_notification.please_enter_subject', { value, location, path });
			}),
		body('pages_descriptions.' + DEFAULT_LANGUAGE_CODE + '.message').optional().trim().notEmpty()
			.withMessage((value, { req, location, path }) => {
				return req.__('admin.email_notification.please_enter_message', { value, location, path });
			}),
